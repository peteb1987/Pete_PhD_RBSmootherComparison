...
for ii = 1:Ns
    ...
    for kk = K-1:-1:2
        ...
        for jj = 1:Nf
            ...
            % KF prediction
            [m_pred, P_pred] = kf_predict(filt_pts_array{kk}(jj).m, filt_pts_array{kk}(jj).P, A, var_z*params.Q);
        ...
        % KF smoothing
        [m_pred, P_pred] = kf_predict(filt_pts_array{kk}(ind).m, filt_pts_array{kk}(ind).P, A, var_z*params.Q);
        Gain = filt_pts_array{kk}(ind).P * A' / P_pred;
